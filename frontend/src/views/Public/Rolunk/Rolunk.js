...
const Rolunk = (props) => {
    ...
    const renderRolunk = () => {
        ...
        return aboutUs && aboutUs.length !== 0 && aboutUs.map((item, index) => {
            let kep = item.kep[0];
            return (  
                <div className={`public_rolunk__item ${isHidden && isHidden.length !== 0 && isHidden[index].isHidden ? 'active' : ''}`} key={index + 'item'}>
                    ...
                    <div className='public_rolunk__alapadatok'>
                        <div>
                           ...
                           {item.email}<br />
                            {item.telefon}<br />
                            <a href="#" hidden={isHidden && isHidden.length !== 0 && isHidden[index].isHidden} onClick={() => toggleRolunk(index)}>Több...</a>
                        </div> 
                    </div>
                </div>
            );
    ...
    const renderLeiras = () => {
        return (
            <React.Fragment>
                <div className='public_rolunk__leiras' id={currentId} hidden={isHidden && isHidden.length !== 0 && !isHidden[currentId].isHidden} dangerouslySetInnerHTML={{__html: isHidden && isHidden.length !== 0 && isHidden[currentId].item.leiras}} />
                <a href="#"  hidden={isHidden && isHidden.length !== 0 && !isHidden[currentId].isHidden} onClick={() => toggleRolunk(currentId)}>Kevesebb...</a>
            </React.Fragment>
        );
    }
