  ...
  if (token) {
    ...
    if (user === null) {
      ...
      if (felvitelObj) {
        ...
        if (user.roles && user.roles.length !== 0 && hasRole(user.roles, "SZUPER_ADMIN")) {
            ...
            users.query(sql, (error) => {
              if (!error) {
                ...
                const resultEmail = await users.query(sqlEmail, [
                  felvitelObj.email,
                ]);
                ...
                if (resultEmail.rowCount === 0) {
                  ...
                  users.query(
                    ...
                    [
                      ...
                      felvitelObj.cim,
                      felvitelObj.telefon,
                      JSON.stringify(felvitelObj.avatar),
                    ...
                    (err) => {
                      if (!err) {
                        res
                          .status(200)
                          .send({ msg: "Felhasználó sikeresen hozzáadva!" });
                      } else {
                        res
                          .status(500)
                          .send({ err: "Felhasználó hozzáadása sikertelen!" });
                      }
                    }
                  );
                } else {
        ...
        res
          .status(400)
          .send({ err: "Felhasználó adatainak megadása kötelező" });
      }
    }
  } else {
    res.status(401).send({ err: "Nincs belépve! Kérem jelentkezzen be!" });
          ...
          if ((user.roles && user.roles.length !== 0 && hasRole(user.roles, "SZUPER_ADMIN")) || user.email === modositoObj.email) {
            users.query(sql, (err) => {
                if (!err) {
                  ...
                  res
                    .status(500)
                    .send({ err: "Felhasználó módosítása sikertelen!" });
                }
              });
          } else {
            res
