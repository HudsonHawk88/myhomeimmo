      ...
      if (
        ...
        if (felvitelObj) {
          ...
          rolunk.query(sql, async (error) => {
            if (!error) {
              const rolunkSql =
                `SELECT azonosito FROM rolunk WHERE azonosito = '${felvitelObj.azonosito}';`;
              ...
              if (result.length === 0) {
                const sql = `INSERT INTO rolunk (azonosito, kep, nev, beosztas, email, telefon, leiras)
                          VALUES ('${felvitelObj.azonosito}', '${JSON.stringify(felvitelObj.kep)}', '${felvitelObj.nev}', '${felvitelObj.beosztas}', '${felvitelObj.email}', '${felvitelObj.telefon}', '${felvitelObj.leiras}');`;
      ...
      if (modositoObj) {
        ...
        if (user.roles && user.roles.length !== 0 && hasRole(JSON.parse(user.roles), ["SZUPER_ADMIN"])) {
          if (id) {
            ...
            const sql = `UPDATE rolunk SET azonosito='${modositoObj.azonosito}', kep='${JSON.stringify(modositoObj.kep)}', nev='${modositoObj.nev}', beosztas='${modositoObj.beosztas}', email='${modositoObj.email}', telefon='${modositoObj.telefon}', leiras='${modositoObj.leiras}' WHERE id = '${id}';`;
            rolunk.query(sql, (err) => {
              if (!err) {
                res
                  .status(200)
