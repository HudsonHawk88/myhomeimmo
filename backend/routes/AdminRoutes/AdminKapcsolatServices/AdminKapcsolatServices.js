      ...
      if (
        ...
        if (felvitelObj) {
          ...
          kapcsolat.query(sql, async (error) => {
            if (!error) {
              const kapcsolatSql =
                `SELECT azonosito FROM kapcsolat WHERE azonosito = '${felvitelObj.azonosito}';`;
              ...
              if (result.length === 0) {
                const sql = `INSERT INTO kapcsolat (azonosito, kep, nev, cim, email, telefon, kapcsolatcim, kapcsolatleiras)
                          VALUES ('${felvitelObj.azonosito}', '${JSON.stringify(felvitelObj.kep)}', '${felvitelObj.nev}', '${felvitelObj.cim}', '${felvitelObj.email}', '${felvitelObj.telefon}', '${felvitelObj.kapcsolatcim}', '${felvitelObj.kapcsolatleiras}');`;
      ...
      if (modositoObj) {
        ...
        if (user.roles && user.roles.length !== 0 && hasRole(JSON.parse(user.roles), ["SZUPER_ADMIN"])) {
          if (id) {
            ...
            const sql = `UPDATE kapcsolat SET azonosito='${modositoObj.azonosito}', kep='${JSON.stringify(modositoObj.kep)}', nev='${modositoObj.nev}', cim='${modositoObj.cim}', email='${modositoObj.email}', telefon='${modositoObj.telefon}', kapcsolatcim='${modositoObj.kapcsolatcim}', kapcsolatleiras='${modositoObj.kapcsolatleiras}' WHERE id = '${id}';`;
            kapcsolat.query(sql, (err) => {
              if (!err) {
                res
                  .status(200)
